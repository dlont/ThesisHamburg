...
\end{equation}
where the magnitude of the strong coupling and PDFs can be chosen independently, without repeating the time-consuming MC integration. Since certain parton configurations lead to identical final-states, only weights for the linear combinations of PDFs $a=\left( \Delta, \Sigma, g\right) $ are stored, where $\Delta=\sum_a{e_q^2\left(q_a\left(x\right)+\bar{q}_a\left(x\right)\right)}$, $\Sigma=\sum_a{q_a\left(x\right)+\bar{q}_a\left(x\right)}$ and $g=g\left(x\right)$ is the gluon PDF. The weights $\mathcal{C}_{n,a,i,j,k}$ are organised in form of a table and can be efficiently processed leading to a jet cross sections calculation time $\mathcal{O}\left( ms\right)$ on a standard CPU\footnote{By 'standard CPU' a single core 1.5GHz CPU is meant.}, to be practical for the usage in the fitting procedure.
